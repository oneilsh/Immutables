...
`node_label` lets us render both leaf values and cached structural measures:
...
Each structural node's label is drawn from its cached `sum` measure — the total
of the leaves beneath it — while each element shows its own value. The
`node$measures` list carries every monoid attached to the tree (including the
built-in `.size` and `.named_count`), so the same pattern generalizes to any
monoid. See `?plot.flexseq` for more details, including handling list-valued
measures like a priority queue's `.pq_min`.
...
The `measure` function receives a structure-specific *entry* — not just the
...
all subsequent accumulated values. This is what enables the O(log n) search.
...
```{r}
loc <- locate_by_predicate(x, function(v) v >= 15, "sum", include_metadata = TRUE)
loc$found
loc$value
loc$metadata
```

The metadata includes `left_measure` (accumulated value before the match),
`hit_measure` (the matched element's own measure), `right_measure` (accumulated
value of everything after), and `index` (1-based position).
...
```{r}
s <- split_by_predicate(x, function(v) v >= 15, "sum")
s$left
s$right
```
...
```{r}
sa <- split_around_by_predicate(x, function(v) v >= 15, "sum")
sa$left
sa$value
sa$right
```
...
wrapper that splits by position using the built-in `.size` monoid:
