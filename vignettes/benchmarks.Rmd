---
title: "Benchmarking Immutables Against Base R and Alternatives"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Benchmarking Immutables Against Base R and Alternatives}
...
# Three run modes, selected by environment variable:
...
#   IMMUTABLES_RUN_FAST=true -> quick re-run for reviewers: only the smallest
#                               few sizes, 1 rep per op. Writes SEPARATE
#                               *-fast.rds / *-fast.pdf files so the canonical
#                               slow results are never overwritten. Finishes in
#                               well under an hour on a regular PC.
#   (neither set)            -> CRAN / ordinary pkgdown build: load the cached
...
# writes its own .pdf.
run_slow <- identical(Sys.getenv("IMMUTABLES_RUN_SLOW"), "true")
run_fast <- identical(Sys.getenv("IMMUTABLES_RUN_FAST"), "true")
run_benchmarks <- run_slow || run_fast

# Fast mode uses a single repetition and only the smallest few sizes per batch;
# `suffix` keeps its outputs (.rds and .pdf) in separate files.
repeats <- if(run_fast) 1L else 10L
suffix  <- if(run_fast) "-fast" else ""
fast_sizes <- function(s) if(run_fast) utils::head(s, 3L) else s
...
# scales::label_timespan() prints microseconds with a Greek mu (U+03BC), which
# the default pdf() device can't render (it drops the glyph -> ".s"). Swap it
# for the micro sign (U+00B5), which lives in the device's ISO-Latin-1
# encoding and renders as a proper "us".
label_time <- function(x) sub("μ", "µ", scales::label_timespan()(x))

# Accumulator populated either by the benchmark chunks (when
# run_benchmarks) or by the loader chunk below (otherwise). One named slot
# per batch: sequence / pq / ordered / ivx.
results_list <- list()
...
.batch_path <- function(batch) {
  file.path(proj_root, "inst", "extdata",
            paste0("benchmarks-", batch, suffix, ".rds"))
}
...
save_figure <- function(plot, file, width, height) {
    ...
    # In fast mode, redirect e.g. benchmarks-sequence.pdf -> benchmarks-sequence-fast.pdf
    file <- sub("\\.pdf$", paste0(suffix, ".pdf"), file)
    ggsave(file.path(out_dir, file), plot, width = width, height = height)
}
```

```{r load-cached, eval=!run_benchmarks, include=FALSE}
for(batch in c("sequence", "pq", "ordered", "ivx")) {
  p <- system.file("extdata", paste0("benchmarks-", batch, ".rds"),
                   package = "Immutables")
  if(nzchar(p) && file.exists(p)) results_list[[batch]] <- readRDS(p)
}
```
...
provided by `Immutables`. Each recorded repetition rebuilds the setup for
size *n*, then times one operation on that fresh state with `bench::mark()`.

**NOTE**: This script takes several hours to run in total as a result of the 
large number of tests and repetitions, run serially with full garbage collection 
between each. The numbers shown below are loaded from cached results shipped with the 
package; re-running the cells in this document regenerates them, as does
executing the script version `data-raw/replication/generate_publication_results.R`.
...
```{r sequence-plot, fig.width=9, fig.height=5}
...
```{r pq-plot, fig.width=9, fig.height=3.2}
...
```{r ordered-bench, eval=run_benchmarks, cache=FALSE}
...
```{r ordered-plot, fig.width=9, fig.height=5}
...
```{r ivx-plot, fig.width=9, fig.height=5}
