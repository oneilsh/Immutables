...
ggplot(seq_results, aes(x = n_cat, y = time_ms, color = impl)) +
...
queue_results <- do.call(rbind, as.list(rows))
```
...
ggplot(queue_results, aes(x = n_cat, y = time_ms, color = impl)) +
  geom_boxplot() +
  facet_wrap(~ op, scales = "free_y") +
  labs(
...
ggplot(pq_results, aes(x = n, y = time_ms, color = impl)) +
...
for(n in ivx_sizes) {
  ...
  ivx_setup  <- function() list(ix = as_interval_index(as.list(vals), start = starts, end = ends, bounds = "[]"))
...
ggplot(ivx_results, aes(x = n, y = time_ms, color = impl)) +
  geom_point(alpha = 0.25, size = 1.2, position = position_jitter(width = 0.03)) +
