...
`lower_bound()` gives *first* element (by index) with a key greater than *or equal to* (`>=`)
a provided query key. `upper_bound()` gives the first with a key *greater than* (`>`)
than a provided query key. Return values indicate if the key is found, the index
of the match, and the associated key and value. The difference in indices thus provides
the count of matched keys, and ...
...
upper_bound(seq, key = "a") |> str()

# greater than all keys
lower_bound(seq, key = "g") |> str()
...
`min_key()` and `max_key()` return the current minimum and maximum *keys* (not the stored elements), in O(log n) and O(1) respectively via cached monoid state.
...
Ordered sequences can carry names, set either at construction or through `as_ordered_sequence()` on a named list. Names and integer positions both support read-only indexing via `[`, `[[`, and `$`; all replacement forms (`[<-`, `[[<-`, `$<-`) error — cast to `as_flexseq()` first to mutate. Named and unnamed elements cannot be mixed within one sequence.
...
`fapply()` maps a function over elements while preserving keys and order. The function receives `(value, key)`, or `(value, key, name)` if it accepts a third argument — keys and names are passed in read-only, and the return value replaces the stored element.
