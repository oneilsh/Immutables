...
are constant time, amortized constant time, or \$O((n))\$ (indexing
\$k\$ elements is \$O(k (n))\$). Core functions are implemented in C++
(via Rcpp) for speed, with matching pure-R reference implementations
